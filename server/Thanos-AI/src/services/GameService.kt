 ...
 */
class GameService(private val server: ProxyServer, private val cmdRouter: CommandRouter, private val playerId:Int) {
    var game = Game()

    // Sub-services
    private val cardService = CardService(server, cmdRouter)
    private val bankService = BankService(server, cmdRouter, game.bank)
    private val playerService = PlayerService(server, cmdRouter, game.players, playerId)
