# Ticket to Ride: Infinity War Edition - CS340

This is a web-based implementation of Ticket to Ride that was created for 
BYU's CS340 class, themed for Infinity War.

There is a server (sometimes) running at [ticket2endgame.jayd.ml](https://ticket2endgame.jayd.ml)

Created early 2019 by Brennon Brimhall, Mark Frame, Jordan Gassaway, Justin Hawkins, and Jayden Milne.

## [client](./client)
Angular 8 based frontend for the client. Typescript/Angular based.

See the readme in that folder for instructions on how to get going.
## [server](./server)
This is the Java/Kotlin backend for the server
